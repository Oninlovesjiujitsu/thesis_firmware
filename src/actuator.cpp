...
void sol1_set(bool open)     { relay_set(SOL1_PIN,    open, "Sol1"); }
...
bool actuator_handle_command(const char *payload, unsigned int length) {
    ...
    if (strstr(payload, "\"PAUSE\"") != NULL) {
        ...
        else if (strstr(payload, "\"PUMP2\""))  pump2_set(on);
        else if (strstr(payload, "\"SOL1\""))   sol1_set(on);
        else if (strstr(payload, "\"SOL2\""))   sol2_set(on);
        else if (strstr(payload, "\"SOL3\""))   sol3_set(on);
        else if (strstr(payload, "\"SOL4\""))   sol4_set(on);
